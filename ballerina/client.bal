...
public isolated client class Client {
    ...
    # + return - A `gcalendar:Calendar` if successful, otherwise an `gcalendar:Error`
    ...
    # + quotaUser - An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
    # + calendarId - Calendar identifier. To retrieve calendar IDs call the calendarList.list method. If you want to access the primary calendar of the currently logged in user, use the "primary" keyword.
    # + return - If successful `()`, otherwise an `gcalendar:Error`
    ...
    # + return - A `gcalendar:Acl` if successful, otherwise an `gcalendar:Error` 
    ...
    # + return - A `gcalendar:AclRule` if successful, otherwise an `gcalendar:Error` 
    ...
    # + return - A `gcalendar:Events` if successful, otherwise an `gcalendar:Error`
    ...
    # + return - A `gcalendar:Event` if successful, otherwise an `gcalendar:Error` 
    ...
    # + prettyPrint - Returns response with indentations and line breaks.
    # + quotaUser - An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
    # + return - A `gcalendar:Colors` if successful, otherwise an `gcalendar:Error`
    ...
    # + prettyPrint - Returns response with indentations and line breaks.
    # + quotaUser - An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
    # + return - A `gcalendar:FreeBusyResponse` if successful, otherwise an `gcalendar:Error`
    ...
    # Learn more about incremental synchronization.
    # Optional. The default is to return all entries.
    # + return - A `gcalendar:CalendarList` if successful, otherwise an `gcalendar:Error` 
    ...
    # + return - A `gcalendar:CalendarListEntry` if successful, otherwise an `gcalendar:Error` 
